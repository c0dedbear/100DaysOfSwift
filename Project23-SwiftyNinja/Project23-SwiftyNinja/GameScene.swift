...
class GameScene: SKScene {
	...
	func createLives() {
		for index in 0..<3 {
			...
			spriteNode.position = CGPoint(x: CGFloat(834 + (index * 70)), y: 720)
			addChild(spriteNode)
	...
	func createSlices() {
		...
		activeSliceBG.strokeColor = .yellow
	...
	func createEnemy(forceBomb: ForceBomb = .random) {
		...
		if enemyType == 0 {
			...
			//ananas code
			enemy = SKSpriteNode(imageNamed: "ananas")
		...
		//position code
		let randomPosition = CGPoint(x: CGFloat.random(in: 64...960), y: -128)
		...
		enemy.physicsBody?.collisionBitMask = 0 // bounce with nothing
	...
	func endGame(triggeredByBomb: Bool) {
		...
		activeEnemies.forEach { $0.isHidden = true }
		livesImages.forEach { $0.isHidden = true }
